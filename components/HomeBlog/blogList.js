...
<section className="blog">
  <div className="container">
    <h2 className="h2 text-center pb-3">Our Blog</h2>
    <div className="row">
    {items.map((item, i) => (
      <div key={i} className="col-xl-4 col-lg-4 col-md-6">
        <div className="blog-col">
          ...
          <div className="blogdtls px-md-3 px-0">
            <div className="blog-titles text-start">{item.name}</div>
            <p className="blog-descri mb-md-4 mb-2 mt-mt-0 mt-2 text-start">{item.shortDescription}</p>
         
          
           <Link className="blackBtn" href={`/blog/${item.slug}`} target="_blank" >View Details</Link>
           </div>
        </div>
      </div>
      ))

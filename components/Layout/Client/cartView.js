...
import { openCart, removeFromCart } from "~/redux/cart.slice";
    ...
    <>
      <li className="dropdown" onClick={() => setShowCart(true)}>
        <Link data-bs-toggle="dropdown" href="#">
          <Image src="/images/icon-cart.svg" width={26} height={26} alt="cats" />
          <span>{getItemsCount()}</span>
        </Link>
        {/* <p>Cart</p> */}
        <ul
          className="dropdown-menu carDropdown"
                ...
                {cartData.items.map((item, index) => (
                  <li key={index} className={c.item}>
                    ...
                    <div className={c.content}>
                      <p>{item.brand} </p>
                      <p>{item.sku} </p>
                      <b>{`${numberFormat(item.price)} (X${item.qty})`}</b>
                    </div>

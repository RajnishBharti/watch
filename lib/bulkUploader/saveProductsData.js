...
const saveProductData = async (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const ProductData = {
      ...
      categories: [convertToSlug(element.categoriesData.category, true)],
      subcategories: convertToSlug(element.categoriesData.subcategories, true),
      brand: convertToSlug(element.brandData.brand, true),
      ...
      specification: Object.entries(element.watchData.specification).map(
        ([title, value]) => ({
          title,
          value,
        })
